...
// Define which views are navigable via URL hash
...
// Helper to get a valid view from the URL hash, defaulting to 'dashboard'
...
const App: React.FC = () => {
  ...
  const [selectedCuttingReportKey, setSelectedCuttingReportKey] = useState<string | null>(null);
  ...
  // Callback for cleaning up page-specific state before navigation
  const cleanupStateForNavigation = useCallback((targetView: View) => {
    if (targetView !== 'orderDetail') {
      setSelectedGroupId(null);
    }
    if (targetView !== 'cuttingReportDetail') {
        setSelectedCuttingReportKey(null);
    }
    // Only clear prefill data if we are NOT navigating to the stock entry page.
    // This preserves it for the one navigation cycle where it's needed.
    if (targetView !== 'stockEntry') {
        setStockEntryPrefill(null);
    }
  }, []);

  // Effect to handle browser navigation (back/forward buttons, manual URL changes)
  useEffect(() => {
    const handleHashChange = () => {
        const viewFromHash = getViewFromHash();
        cleanupStateForNavigation(viewFromHash);
        setActiveView(viewFromHash);
    };

    window.addEventListener('hashchange', handleHashChange);
    return () => window.removeEventListener('hashchange', handleHashChange);
  }, [cleanupStateForNavigation]);
  
  // Effect to clean up editing state when navigating away from edit views
  useEffect(() => {
  ...
  // Function for programmatic navigation (e.g., clicking a link)
  const navigateTo = (view: View) => {
    // Setting the hash will trigger the 'hashchange' listener, which handles state updates.
  ...
  const ordersForEditingReport = useMemo(() => {
    ...
    // Return all orders that share the same groupId
  ...
  const handleSelectCuttingReport = (key: string) => {
    setSelectedCuttingReportKey(key);
    navigateTo('cuttingReportDetail');
  };

  const handleEditItem = (item: EditableItem) => {
    setEditingItem(item);
    if (item.hasOwnProperty('groupId')) { // Order
        navigateTo('editOrder');
    } else if (Array.isArray(item)) { // CuttingReport[]
        navigateTo('editCuttingReport');
    } else if (item.hasOwnProperty('defectiveSizes')) { // StockEntry
        navigateTo('editStockEntry');
    }
  };
  
  const handleCancelEdit = () => {
    setEditingItem(null);
    // Determine which list view to return to
    ...
    else navigateTo('dashboard'); // Fallback
  };
  ...
  const handleUpdateCuttingReport = async (reports: CuttingReport[], sizes: Sizes, date: string) => {
      ...
      handleCancelEdit(); // Navigate back after update is complete
  };
  ...
  const renderActiveView = () => {
    switch (activeView) {
      ...
      case 'orderDetail': {
        ...
        if (selectedOrderGroup.length === 0) {
            setTimeout(() => navigateTo('orders'), 0);
            return null;
        }
        return <OrderDetail orders={selectedOrderGroup} stockUsage={stockUsage} cuttingReports={cuttingReports} stockEntries={stockEntries} producers={producers} onClose={handleCloseDetail} onReassign={reassignMultipleParts} onViewCuttingReport={() => navigateTo('cuttingReport')} addToast={addToast} />
      }
      case 'cuttingReport':
      ...
      case 'cuttingReportDetail': {
        ...
        if (reportGroup.length === 0) {
            setTimeout(() => navigateTo('cuttingReport'), 0);
            return null;
        }
      ...
      case 'stockEntry':
        return <StockEntryForm colors={colorNames} defectReasons={defectReasons} onSave={addStockEntries} addColor={addColor} prefillData={stockEntryPrefill} onBack={() => navigateTo('stockRecords')} addToast={addToast} />;
      case 'editOrder':
  ...
  return (
    <div className="flex min-h-screen bg-background text-text-primary">
      <Sidebar activeView={activeView} setActiveView={navigateTo} theme={theme} toggleTheme={toggleTheme} isOpen={isSidebarOpen} setIsOpen={setSidebarOpen} />
