    ...
    useEffect(() => {
        ...
        stockUsage.forEach(su => {
            ...
            if (order) {
                ...
                if (stockEntry) {
                    ...
                    // FIX: Cast stockEntry to its type to allow property access.
        ...
        orders.forEach(order => {
            ...
            if (newStatus !== order.status || newCompletionDate !== order.completionDate) {
                ordersToUpdate.push({ ...order, status: newStatus, completionDate: newCompletionDate });
            }
        });
            ...
            const updatedOrderIds = new Set(ordersToUpdate.map(o => o.id));
             setData(currentData => ({ 
                ...
                orders: currentData.orders.map(o => {
                    const updated = ordersToUpdate.find(uo => uo.id === o.id);
                    return updated || o;
                }) 
            }));
            // This is a derived state; we'll let the server handle persisting the final state from other actions.
        }
    }, [orders, stockUsage, cuttingReports, stockEntries, isLoaded]);

    const stats = useMemo(() => {
        const uniqueGroupIds = new Set(orders.map(o => o.groupId));
