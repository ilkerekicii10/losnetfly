...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, theme, toggleTheme, isOpen, setIsOpen }) => {
  const navItems = [
    { id: 'dashboard', label: 'Anasayfa', icon: DashboardIcon },
    { id: 'orders', label: 'Sipariş Listesi', icon: ListIcon },
    { id: 'orderEntry', label: 'Emir Girişi', icon: PlusCircleIcon },
    { id: 'cuttingReport', label: 'Kesim Raporu', icon: CuttingIcon },
    { id: 'stockEntry', label: 'Stok Girişi', icon: PlusCircleIcon },
    { id: 'stockRecords', label: 'Stok Kayıtları', icon: BoxIcon },
    { id: 'workshopManagement', label: 'Atölye Yönetimi', icon: UsersIcon },
    { id: 'defectManagement', label: 'Defo Detayları', icon: TagIcon },
  ];
  
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, view: View) => {
    // Let the browser handle new tab/window clicks (Cmd/Ctrl+Click, Middle click)
    if (e.metaKey || e.ctrlKey || e.button === 1) {
      return;
    }
    e.preventDefault(); // Prevent default link behavior for SPA-style navigation
    setActiveView(view); // Call the navigation function from App.tsx
    setIsOpen(false); // Close sidebar on navigation on mobile
  };


  return (
    <aside className={`w-64 bg-surface flex-shrink-0 border-r border-border-color flex flex-col transform transition-transform duration-300 ease-in-out fixed lg:sticky lg:top-0 lg:h-screen inset-y-0 left-0 z-50 
        ...
        <ul>
          {navItems.map((item) => (
            <li key={item.id}>
              <a
                href={`#${item.id}`}
                onClick={(e) => handleLinkClick(e, item.id as View)}
